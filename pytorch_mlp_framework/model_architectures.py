...
class ConvolutionalProcessingBlockBN(nn.Module):
    ...
    def build_module(self):
        ...
        out = self.layer_dict['conv_0'].forward(out)
        #init 2d batch norm layer which takes number of channels from (N,C,H,W) for it's num_features argument
...
class ConvolutionalProcessingBlockBNRC(nn.Module):
    ...
    def build_module(self):
        #no change in build model needed due to zero init but the skip connection could be useful for other init
        self.layer_dict = nn.ModuleDict()
        x = torch.zeros(self.input_shape)
        x_shortcut=x
        out = x
